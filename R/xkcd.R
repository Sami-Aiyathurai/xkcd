...
#'
#' Given an xkcd comic number, this function retrieves a JSON object descripbing that cmcic from the
#' xkcd website
#'
#' @param number A scalar numeric vector idenitifying an xkcd comic number
...
validate_xkcd <- function(x){
  ...
  if (!all(required_fields %in% names(x))) {
    ...
    stop("xkcd object is missing the following required fields: ", paste(difference, collapse = ", "))
  }
  ...
  for (f in char_fields){
    if (!(is.character(x[[f]]) && length(x[[f]]) == 1)) {
      stop("This", f, "field in an xkcd object must be a character vector of length 1")
    }
  }
